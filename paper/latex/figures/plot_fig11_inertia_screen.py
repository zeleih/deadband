...
def _draw_heatmap(ax, data: np.ndarray, title: str, *, show_ylabels: bool,
    ...
    ax.set_title(title, pad=4, fontsize=8.2)
    ax.set_xticks(range(len(INERTIA_ORDER)))
    ax.set_xticklabels([f"{x:g}x" for x in INERTIA_ORDER])
    ...
    ax.set_xlabel("synchronous inertia multiplier")
    ...
    cbar = ax.figure.colorbar(im, ax=ax, fraction=0.040, pad=0.022)
    cbar.set_ticks([-span, -span / 2, 0, span / 2, span])
    cbar.ax.tick_params(labelsize=6.6, width=0.5, length=2.3)
...
def main() -> None:
    ...
    fig, (ax_tail, ax_edge) = plt.subplots(
        1, 2, figsize=(7.05, 2.22), gridspec_kw={"wspace": 0.14}
    )
    _draw_heatmap(
        ax_tail,
        tail,
        r"(a) Tail-risk reduction: $|\Delta f|>50$ mHz",
        show_ylabels=True,
        span_override=100.0,
    )
    _draw_heatmap(
        ax_edge,
        edge,
        r"(b) Boundary-mass reduction: $\mathrm{EM}_{36}$",
        show_ylabels=False,
        span_override=30.0,
    )
    fig.subplots_adjust(left=0.11, right=0.985, bottom=0.20, top=0.86)
